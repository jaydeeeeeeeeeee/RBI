<?php
function ul(FPDF $pdf, float $x, float $y, float $w, float $h, string $txt, string $align = 'L'): void {
    $pdf->SetXY($x, $y);
    $pdf->Cell($w, $h, $txt, 0, 0, $align);
    $pdf->Line($x, $y + $h, $x + $w, $y + $h);
}

function draw_code128(FPDF $pdf, string $text, float $x, float $y,
                      float $barH = 12, float $narrowW = 0.38): void
{
    // Code 128B
    $patterns = [
    ...
    $bits .= $patterns[$checksum] ?? '00000000000';
    $bits .= $stopPattern;
    ...
    for ($i = 0; $i < strlen($bits); $i++) {
        if ($bits[$i] === '1') {
            $pdf->Rect($curX, $y, $narrowW, $barH, 'F');
        }
        $curX += $narrowW;
    }
}